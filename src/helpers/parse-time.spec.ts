...
describe('parse time', () => {
  ...
  describe('spacing', () => {
    it('spaces around the values are valid', () => {
      expect(parseTime(' 20m ')).toBe(20);
    });

    it('spaces before identifiers are valid', () => {
      expect(parseTime('20 m')).toBe(20);
    });

    it('spaces between values are valid', () => {
      expect(parseTime('5h   10m')).toBe(310);
    });
  });

  describe('not valid format', () => {
    it('is NaN if value is incorrect', () => {
      expect(parseTime('world')).toBeNaN();
    });

    it('number without identifier is not valid', () => {
      expect(parseTime('20')).toBeNaN();
    });

    it('few modifiers of same type', () => {
      expect(parseTime('2d 10h 1h')).toBeNaN();
      expect(parseTime('10m 1m')).toBeNaN();
      expect(parseTime('20d 1d 1m')).toBeNaN();
    });

    it('bigger time chunk used after lower', () => {
